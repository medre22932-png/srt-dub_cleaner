...
def clean_brackets_and_citations(text):
    text = re.sub(r'\[.*?\]', '', text)
...
def replace_surah_parens(text):
    def repl(match):
        ...
        # NEW: Check for numerical Surah:Ayah formats (e.g., 55:7)
...
def process_text_for_srt(text):
    text = clean_brackets_and_citations(text)
...
def choose_split_index(tokens, start_idx=0, min_words=20, hard_limit=42):
    remaining = len(tokens) - start_idx
    if remaining <= min_words:
        return None

    min_split = start_idx + min_words
    max_scan = min(len(tokens), start_idx + hard_limit)

    strong_candidate = None
    weak_candidate = None

    for i in range(min_split - 1, max_scan):
        tok = tokens[i][0]
        if token_has_strong_punctuation(tok):
            strong_candidate = i + 1
            break
        if weak_candidate is None and token_has_split_punctuation(tok):
            weak_candidate = i + 1
...
def split_text_robust(text, min_words=20, hard_limit=42):
    ...
    while start_idx < len(tokens):
        split_idx = choose_split_index(tokens, start_idx, min_words=min_words, hard_limit=hard_limit)
...
def extend_timestamps_to_next(blocks):
    """
    NEW: Extends the end timestamp of each block to match the start timestamp of the next block.
    """
...
def build_output_srt(blocks):
    ...
    for block in blocks:
        ...
        parts = split_text_robust(cleaned, min_words=20, hard_limit=42)
    ...
    # Apply the new extension rule to snap timestamps together
